...
describe('Swarm City API socket client > test sendSignedTransaction', function() {
	let client;
	let socketURL;
	...
	it('should connect and call sendSignedTransaction', function(done) {
		logger.info('connecting to %s', socketURL);
		client = io.connect(socketURL, options);

		let promises = [];
		promises.push(new Promise((resolve, reject) => {
			client.emit('sendSignedTransaction', {
				...
				logger.info('sendSignedTransaction returned', reply);
